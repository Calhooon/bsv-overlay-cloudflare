...
/// absent a re-latch pass**, tracked as bsv-low#355 — which is scoped to
/// EVERY row, not just the `NULL` ones, because a rank-0 row is equally
/// unrecoverable and nothing re-evaluates the column (see the module doc's
/// "a `0` is a verdict, not 'not yet checked'"). What heals in practice is
...
///
/// "Once" is literal, and was not until the second gate (round 2, LOW-2):
/// the D1 writer used to evaluate this twice per admitted marker — once for
/// the `[potparty:siginvalid]` telemetry and once for the bind — while this
/// comment claimed otherwise. The verdict is now computed inside
/// `potparty_write::potparty_insert_query` and carried on the insert, so the
/// telemetry reports the value actually written. Pinned by
/// `exactly_one_potparty_insert_statement_exists_in_this_module`.
///
