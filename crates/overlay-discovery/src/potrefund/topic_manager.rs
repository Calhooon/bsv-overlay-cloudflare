//! POTREFUND Topic Manager — validates LOW pre-signed refund-backup markers.
//!
//! See [`super`] for the full on-wire format. One record version is
//! admitted: the `LOW/potrefund/v1` `OP_RETURN` data carrier (7 pushes: tag
//! / identity / gameId / potTxid / potVout / refundRawHex / sig).
//!
//! Like `tm_result` / `tm_collected` / `tm_potparty`, there is NO
//! server-side signature verification and NO transaction validation: the
//! marker is admitted by BYTE FORMAT ONLY. The `refundRawHex` + `sig` it
//! carries are preserved verbatim and parsed / verified CLIENT-side if at
//! all. The overlay is an INDEX, not an authority.
...
use super::is_potrefund_marker_script;

/// POTREFUND Topic Manager — identifies admissible LOW potrefund markers.
pub struct PotrefundTopicManager;

impl PotrefundTopicManager {
    /// Create a new POTREFUND Topic Manager.
    pub fn new() -> Self {
        Self
    }
    ...
    pub fn validate_potrefund_output(output: &bsv_rs::transaction::TransactionOutput) -> bool {
        is_potrefund_marker_script(&output.locking_script.to_binary())
    }
}

impl Default for PotrefundTopicManager {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait(?Send)]
impl TopicManager for PotrefundTopicManager {
    async fn identify_admissible_outputs(
        ...
        for (i, output) in tx.outputs.iter().enumerate() {
            if Self::validate_potrefund_output(output) {
                debug!("POTREFUND: admitted output {i}");
                outputs_to_admit.push(i as u32);
            }
            // Not a potrefund marker (change P2PKH, foreign OP_RETURN, a
            // malformed tag, …) — skip. The strict byte-format parse keeps
            // junk out of the index.
        }

        if outputs_to_admit.is_empty() {
            warn!("POTREFUND: no outputs admitted");
        }
    ...
    async fn get_documentation(&self) -> String {
        include_str!("../../docs/potrefund_topic.md").to_string()
    }

    async fn get_metadata(&self) -> ServiceMetadata {
        ServiceMetadata {
            name: "POTREFUND Topic Manager".to_string(),
            description: Some(
                "Indexes LOW pre-signed refund-backup markers \
                 (LOW/potrefund/v1 OP_RETURN) keyed by pot outpoint for \
                 keyless recovery re-broadcast."
                    .to_string(),
            ),
            ..Default::default()
        }
...
pub(crate) mod tests {
    use super::super::tests::{
        ...
        golden_vout, push_data,
    };
    use super::super::POTREFUND_TAG;
    use super::*;
    use bsv_rs::script::LockingScript;
    use bsv_rs::transaction::{Transaction as Tx, TransactionInput, TransactionOutput};

    /// A valid potrefund-marker `TransactionOutput` (0-sat OP_RETURN).
    pub(crate) fn make_potrefund_output(game_id: &[u8; 32]) -> TransactionOutput {
    ...
    fn valid_marker_admitted() {
        let out = make_potrefund_output(&[0x42u8; 32]);
        assert!(PotrefundTopicManager::validate_potrefund_output(&out));
    }
    ...
    fn p2pkh_not_admitted() {
        ...
        assert!(!PotrefundTopicManager::validate_potrefund_output(&output));
    }
    ...
    #[test]
    fn empty_refund_not_admitted() {
        // refundRawHex empty — the refund backup carries nothing.
        let script = super::super::tests::marker_script(
            &golden_identity(),
            &golden_game_id(),
            &golden_pot_txid(),
            golden_vout(),
    ...
    fn malformed_tagged_marker_not_admitted() {
        // potrefund-TAGGED but a short gameId — strict lengths reject it.
        let mut s = vec![0x00, 0x6au8];
    ...
    async fn identify_admissible_outputs_over_beef() {
        let marker = make_potrefund_output(&golden_game_id());
        ...
        let mgr = PotrefundTopicManager::new();
    ...
    async fn topic_manager_trait_works() {
        ...
        assert_eq!(meta.name, "POTREFUND Topic Manager");
