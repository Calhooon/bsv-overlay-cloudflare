...
use low_app_layer::hops_view::{
    apply_chain_probes, assemble_hops_view, expected_hop_lock_hex, hops_view_body, hops_view_sql,
    ChainSpendProbe, HopStatus, HopsViewRow, MarkerVerification, HOPS_VIEW_MAX_OUTPOINTS,
};
...
fn real_marker_in_its_production_container_is_verified_and_unspent() {
    ...
    assert_eq!(
        e.status_source,
        Some("index"),
        "B2.1: the index's non-observation is labelled index, never chain"
    );
    assert_eq!(
        e.marker_verified,
...
fn b21_a_hop_swept_outside_the_overlay_is_served_spent_once_the_chain_rung_says_so() {
    ...
    assert_eq!(
        entries[0].status_source,
        Some("index"),
        "the index's word is labelled index"
    );
    ...
    assert_eq!(
        served[0].status,
        HopStatus::Spent,
        "the chain rung's confirmed spender wins"
    );
    ...
    assert!(
        body.contains("\"status\":\"spent\""),
        "the wire says spent: {body}"
    );
...
/// bsv-low #451 slice C (iii): the verdict memo's SQL against the REAL schema (migration 151).
#[test]
fn verdict_memo_round_trips_on_the_production_schema() {
