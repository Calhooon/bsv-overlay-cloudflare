...
fn every_fixed_query_prepares_against_the_production_schema() {
    ...
    assert_prepares(&conn, "RESULT_CHAIN_ROWS_SQL", RESULT_CHAIN_ROWS_SQL);
    assert_prepares(&conn, "COLLECTED_CHAIN_ROWS_SQL", COLLECTED_CHAIN_ROWS_SQL);
    assert_prepares(&conn, "POTREFUND_LATCH_SQL", POTREFUND_LATCH_SQL);
    assert_eq!(
        lb_row_file_sql(),
