...
pub async fn complete_pot_beef_proofs(
    ...
    let mut latched: Vec<(String, Option<u64>)> = Vec::new();
    ...
    for (txid, stored_beef) in candidates {
        ...
        if let Some(bump_hex) = stored_bump {
            if fetcher.verify_proof(&txid, &bump_hex).await {
                ...
                // revalidation sweep can window this row later
                let anchor = bsv_rs::transaction::MerklePath::from_hex(&bump_hex)
                    .ok()
                    .map(|mp| u64::from(mp.block_height));
                latched.push((txid, anchor));
                continue;
            }
            push_log(&format!(
    ...
    // not silently short).
    if !latched.is_empty() {
        match pot_storage.mark_pot_beefs_proven_at(&latched).await {
...
pub async fn apply_pushed_proof_to_pot_stores(
    ...
    let already_verified = pot_storage
        ...
        .await
        .unwrap_or(false);
    // bsv-low M19 R2: the block this (route-verified) push names, as an
    // ANCHOR (height + the root the bump computes; round-2 review M2). A
    // stored VERIFIED bump that names a DIFFERENT block, by height OR by
    // root (a tx in both competing blocks keeps its height), is an orphan's
    ...
    match pot_storage.get_beef(txid).await {
        Ok(Some(stored_beef)) => {
            let stored_anchor = crate::reorg_sweep::stored_bump_anchor(&stored_beef, txid);
            let reanchor = already_verified
                && pushed_anchor.as_ref().is_some_and(|p| {
                    overlay_discovery::pot::reorg::is_reanchor(stored_anchor.as_ref(), p)
                });
                    ...
                    Some(compacted) => match pot_storage.compact_pot_beef(txid, &compacted).await {
                        Ok(()) => {
                            ...
                            if reanchor {
                                ...
                                push_log(&format!(
                                    "[arc-ingest] {txid} pot-beef RE-ANCHORED {stored_anchor:?} → {pushed_anchor:?} (the stored bump named an orphan's block)"
                                ));
                            }
                        }
                        Err(e) => {
    ...
    if let Some(new_height) = pushed_height {
        match pot_storage.find_confirmed_by_spending_txid(txid).await {
            Ok(records) => {
                for rec in records {
                    // a row holds only a height: the same height is the same
                    // anchor for it (the stored BUMP above is what a same-height
                    // root change replaces); a height-less row is the confirm
                    // arm's fill, never a re-anchor
                    if rec.spent_height.is_none() || rec.spent_height == Some(new_height) {
                        continue;
                    }
                    match pot_storage
...
pub(crate) mod tests {
    ...
    pub(crate) fn single_tx_bump(txid: &str, height: u32) -> MerklePath {
    ...
    pub(crate) fn real_spender_raw(pot_txid: &str, vout: u32) -> String {
